...
void delete(char iD[50])
{
    ...
    FILE *fptr1, *fptr2;
    fptr1 = fopen("Password.txt", "r");
    fptr2 = fopen("tempin.txt", "w");
    fseek(fptr1,0,SEEK_SET);  // Takes cursor in file to first position

    for(int k = 0 ; k < pinl ; k++) // Copies the contents of Password.txt to Temp.txt
    {
        fscanf(fptr1,"%d",&garb); // Inputs one line into garbage variable
        if(k == pinpos) 
        {
            // Does not Copy contents of the pin we want to delete
            continue;
        }
        fprintf(fptr2,"%d\n", garb); // Inserts the line from Password.txt into Temp.txt

    }
    fclose(fptr1);
    fclose(fptr2);
    remove("Password.txt"); // removes file with old pin
    rename("tempin.txt", "Password.txt");
    // replaces new file without old pin which was deleted
}
